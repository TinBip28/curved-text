...
def uncurve_text_tight(input_path, output_path, n_splines, arc_equal=False, return_figures=False):
    ...
    if return_figures:
        figures.append(("Văn Bản với Đường Cong Phát Hiện", fig1))
    else:
        plt.show()
    ...
    for points in perpendicular_points:
        ...
        my_iter += 1
    # Plot perpendicular lines visualization
    fig2 = plt.figure(figsize=(10, 5))
    plt.imshow(thresh, cmap='gray', extent=[0, thresh.shape[1], thresh.shape[0], 0])
    plt.plot(X_new, y_hat, color='red')
    for points in perpendicular_points[::10]:  # Plot every 10th line for clarity
        plt.plot([x[0] for x in points], [x[1] for x in points], color='blue', alpha=0.5)
    plt.axis('off')
    plt.subplots_adjust(bottom=0, left=0, right=1, top=1)
    if return_figures:
        figures.append(("Đường Lấy Mẫu Vuông Góc", fig2))
    else:
        plt.show()
    ...
    plt.subplots_adjust(bottom=0, left=0, right=1, top=1)
    if return_figures:
        figures.append(("Kết Quả Giai Đoạn 1", fig3))
    else:
        plt.show()
...
def uncurve_text(input_path, output_path, n_splines, arc_equal=False, return_figures=False):
    # Load image, grayscale it, Otsu's threshold
    image = cv2.imread(input_path)
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    thresh = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1]
    ...
    if len(black_pixels) == 0:  # No black pixels found
        print("Warning: No black pixels found in the image. Using full image range.")
        leftmost_x, rightmost_x = 0, image.shape[1]-1
        X = np.array([i for i in range(image.shape[1])]).reshape(-1, 1)
        y = np.ones(X.shape[0]) * (image.shape[0] // 2)
    else:
    ...
    plt.subplots_adjust(bottom=0, left=0, right=1, top=1)
    if return_figures:
        figures.append(("Phát Hiện Đường Cong Giai Đoạn 2", fig1))
    else:
        plt.show()
    ...
    for i in range(leftmost_x, rightmost_x + 1):
        idx = i - leftmost_x
        if idx < len(y_hat):
            roll_amount = round(y_hat[idx] - thresh.shape[0]/2)
            image[:, i, 0] = np.roll(image[:, i, 0], roll_amount)
            image[:, i, 1] = np.roll(image[:, i, 1], roll_amount)
            image[:, i, 2] = np.roll(image[:, i, 2], roll_amount)
    ...
    plt.subplots_adjust(bottom=0, left=0, right=1, top=1)
    if return_figures:
        figures.append(("Kết Quả Cuối Cùng", fig2))
    else:
        plt.show()
